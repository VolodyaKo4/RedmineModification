...
<<<<<<< HEAD
    QueryColumn.new(:parent_issue_status, :default_order => 'desc'),
    QueryColumn.new(:parent_issue_project, :default_order => 'desc'),
=======
...
<<<<<<< HEAD
    QueryColumn.new(:due_date_month, :sortable => "#{Issue.table_name}.due_date", :groupable => "cast(date_trunc('month', #{Issue.table_name}.due_date) as date)"), # this group statement works only for postgresql cause of for all SQL servers date functions are different
=======
>>>>>>> 248a796 (pure version 2.5.1 from http://www.redmine.org/releases/redmine-2.5.1.tar.gz)
...
<<<<<<< HEAD
    add_available_filter "parent_issue_status",
      :type => :list_status, :values => IssueStatus.sorted.collect{|s| [s.name, s.id.to_s] }

#    if project.nil?
# because of we add one more filter for project, and it does not depend on
# the list for one project or all projects
# we will collect project values always but add old project filter by the old condition (it is moved bellow)    
=======
    if project.nil?
>>>>>>> 248a796 (pure version 2.5.1 from http://www.redmine.org/releases/redmine-2.5.1.tar.gz)
      project_values = []
...
<<<<<<< HEAD
      add_available_filter("parent_issue_project",
        :type => :list, :values => project_values
      ) unless project_values.empty?
=======
>>>>>>> 248a796 (pure version 2.5.1 from http://www.redmine.org/releases/redmine-2.5.1.tar.gz)
...
<<<<<<< HEAD
      ...
      index = (index ? index + 1 : -1)
      # insert the column after estimated_hours or at the end
      @available_columns.insert index, QueryColumn.new(:estimated_hours_of_leaves,
        #TODO :sortable
        :default_order => 'desc',
        :caption => :field_estimated_hours_of_leaves
      )
      ...
      @available_columns.insert index, QueryColumn.new(:total_spent_hours,
        #TODO :sortable
        :default_order => 'desc',
        :caption => :field_total_spent_hours
      )
...
<<<<<<< HEAD
  ...
  def issue_sum
    Issue.visible.sum(:estimated_hours, :include => [:status, :project],
    ...
    if grouped?
      begin
        ...
        r = Issue.visible.
          ...
          count
<<<<<<< HEAD
          # in postgresql when data functions is used they returns string type
          # convert to date if there groupped by 'date' field
          r2 = {}
          if group_by_statement.include?('date') 
            r.each do |key, value|
              if key.is_a? String  
                key2 = Date.parse(key)
                r2[key2] = value
...
<<<<<<< HEAD
  ...
  def issue_sum_by_group
    ...
    if grouped?
      begin
        r = Issue.visible.sum(:estimated_hours, :joins => joins_for_order_statement(group_by_statement), :group => group_by_statement, :include => [:status, :project], 
         :conditions => statement << " AND #{Issue.table_name}.rgt - #{Issue.table_name}.lft >= 1")
        # in postgresql when data functions is used they returns string type
        # convert to date if there groupped by 'date' field
        # FIXME copy-paste from issue_count_by_group above 
        r2 = {}
        if group_by_statement.include?('date') 
          r.each do |key, value|
            if key.is_a? String  
...
<<<<<<< HEAD
  def sql_for_parent_issue_status_field(field, operator, value)
    va = value.collect{|val| "'#{connection.quote_string(val)}'"}.join(",")
    "#{Issue.table_name}.parent_id IN (SELECT #{Issue.table_name}.id FROM #{Issue.table_name} WHERE #{Issue.table_name}.status_id #{ operator == '=' ? 'IN' : 'NOT IN' } (#{va}) " + ')'
  end

  def sql_for_parent_issue_project_field(field, operator, value)
    va = value.collect{|val| "'#{connection.quote_string(val)}'"}.join(",")
    "#{Issue.table_name}.parent_id IN (SELECT #{Issue.table_name}.id FROM #{Issue.table_name} WHERE #{Issue.table_name}.project_id #{ operator == '=' ? 'IN' : 'NOT IN' } (#{va}) " + ')'
  end
