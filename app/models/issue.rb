...
<<<<<<< HEAD
    self.attributes = issue.attributes.dup.except("id", "root_id", "parent_id", "lft", "rgt", "created_on", "updated_on", "issue_number")
=======
...
<<<<<<< HEAD
  def estimated_hours_of_leaves
    @estimated_hours_of_leaves ||= leaves.sum(:estimated_hours) || 0
  end

  def due_date_month
    due_date.at_beginning_of_month unless due_date.nil?
  end
...
<<<<<<< HEAD
  def parent_issue_status
    parent_id.nil? ? parent_id : Issue.find_by_id(parent_id).status
  end

  def parent_issue_project
    parent_id.nil? ? parent_id : Issue.find_by_id(parent_id).project
  end
...
<<<<<<< HEAD
        # p.priority = IssuePriority.find_by_position(priority_position) # now user updates the field for issues with subtasks manually
      end

      # start/due dates = lowest/highest dates of children
      # p.start_date = p.children.minimum(:start_date) # now user updates the field for issues with subtasks manually
      # p.due_date = p.children.maximum(:due_date) # now user updates the field for issues with subtasks manually
=======
...
<<<<<<< HEAD
      # p.estimated_hours = p.leaves.sum(:estimated_hours).to_f # now user updates estimated time for issues with subtasks manually 
=======
