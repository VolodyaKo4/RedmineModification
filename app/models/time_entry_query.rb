...
<<<<<<< HEAD
  def sql_for_assigned_to_id_field(field, operator, value)
    if (operator == '*') || (operator == "!*") # any or none
      query_statement = "(#{Issue.table_name}.assigned_to_id IS #{ (operator == '*') ? 'NOT' : '' } NULL)"
    else
      va = value.collect{|val| "'#{connection.quote_string(val)}'"}.join(",")
      query_statement = "(#{Issue.table_name}.assigned_to_id #{ (operator == '=') || (operator == '*') ? 'IN' : 'NOT IN' } (#{va}) " + ')'
    end
    query_statement
  end
