...
<<<<<<< HEAD
       ...
       def issues_to_pdf_full(issues, project, query)
         ...
         issues.each do |issue|
           issue_to_pdf_content(pdf, issue, :journals => issue.journals)
         end
         pdf.Output
      end
...
<<<<<<< HEAD
         ...
         issue_to_pdf_content(pdf, issue, assoc)
         pdf.Output
       end

       def issue_to_pdf_header(pdf, issue)
	begin
		issue_id = issue.issue_id.to_s
	rescue
		issue_id = issue.id
	end
	pdf.SetTitle("#{issue.project} - #{issue.tracker} ##{issue_id}")
        pdf.alias_nb_pages
        pdf.footer_date = format_date(Date.today)
       end
 
       def issue_to_pdf_content(pdf,issue, assoc={})
  begin
    issue_id = issue.issue_id.to_s
  rescue
    issue_id = issue.id
        ...
        pdf.AddPage
        pdf.SetFontStyle('B',11)
        buf = "#{issue.project} - #{issue.tracker} ##{issue_id}"
=======
...
<<<<<<< HEAD
            begin
              child_id = child.issue_id.to_s
            rescue
              child_id = child.id
            end
            buf = "#{child.tracker} # #{child_id}: #{child.subject}".
=======
